...
res = 0.0000001  #Convergence criterion
#"""
...
for j in range(maxitr):  
    ...
    for i in range(N):#Stochastic Gradient Descent-Ada grad
        dJ = np.dot(np.transpose(x[i,:]),(np.dot(x[i],w)  - y[i]))/N #grad J
        Gjj+=dJ**2 #sum of squared gradients till ith step
        dumg = 1/Gjj**(0.5) #calculation for easier implementation in next line
        w = w -a*dumg*dJ #updating weights 
